...
/**
 * Extends The Base Worker Responsible for the ETH network
 */
export class EthereumWorker extends BaseChainWorker {
  constructor(protected targeBlock: number) {
    super(getChainConfig("ethereum"), targeBlock);
  }
  ...
  protected logEventBatch(fromBlock: number, toBlock: number, logs: LogData[], fetchTimeMs: number): void {
    console.log(`[ETH] Events ${fromBlock}-${toBlock} | Count: ${logs.length} | ${fetchTimeMs}ms`);
  }
}
